...
async def register_face(
    ...
    """
    Register a face image for a specific pose
    Frontend sends: { employee_id, image (base64), pose }
    """
    try:
        employee_code = request.get('employee_id')
        image_base64 = request.get('image')
        pose = request.get('pose')
        
        if not all([employee_code, image_base64, pose]):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Missing required fields: employee_id, image, pose"
            )
        ...
        if len(faces) > 1:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Multiple faces detected. Please ensure only one face in frame"
            )
        ...
        employee_dir = os.path.join(settings.EMPLOYEE_IMAGES_PATH, employee_code)
        os.makedirs(employee_dir, exist_ok=True)
        
        timestamp = int(time.time())
        pose_dir = os.path.join(employee_dir, f"{employee_code}_{timestamp}")
        os.makedirs(pose_dir, exist_ok=True)
        
        image_path = os.path.join(pose_dir, f"{pose}.jpg")
        cv2.imwrite(image_path, frame)
        
        # Update employee embeddings
        current_embeddings = json.loads(employee.embeddings) if employee.embeddings else []
        current_embeddings.append(embedding.tolist())
        
        employee.embeddings = json.dumps(current_embeddings)
        employee.total_embeddings = len(current_embeddings)
        
        # Calculate mean embedding
        all_embeddings = np.array(current_embeddings)
        mean_embedding = np.mean(all_embeddings, axis=0)
        employee.mean_embedding = json.dumps(mean_embedding.tolist())
        
        db.commit()
        db.refresh(employee)
        
        logger.info(f"✅ Registered face for {employee_code} - pose: {pose} - total: {employee.total_embeddings}")
        
        return {
            "success": True,
            "message": f"Face registered successfully for pose: {pose}",
            "employee_code": employee_code,
            "pose": pose,
            "total_embeddings": employee.total_embeddings
        }
